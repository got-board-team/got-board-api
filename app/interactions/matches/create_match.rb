class CreateMatch < ActiveInteraction::Base
  integer :players_count, default: 6
  ...
  def execute
    ActiveRecord::Base.transaction do
      ...
      create_players(match, players_count)
  ...
  def create_stark_player(match)
    ...
    board = match.board
    Boat.create!(player: player, board: board, territory: "the_shivering_sea", x: 1208, y: 738)
    Knight.create!(player: player, board: board, territory: "winterfell", x: 625, y: 851)
    Footman.create!(player: player, board: board, territory: "winterfell", x: 773, y: 544)
    Footman.create!(player: player, board: board, territory: "white_harbor", x: 852, y: 810)
    player
  end
